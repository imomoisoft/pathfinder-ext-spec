...
describe 'Запрос информации о документах удостоверяющих личность.' do
  ...
  let(:true_serie) { '7000' }
  let(:true_number) { '900000' }

  let(:true_first_name) { 'Георгий' }
  let(:true_last_name) { 'Фрунзе' }
  let(:true_middle_name) { 'Васильевич' }

  let(:true_issue_date) { DateTime.new(2001, 5, 11) }
  let(:true_issuer) { 'Организация выдающая паспорта РФ' }
  ...
  context 'Информация о российском пасспорте' do
    subject { @doc.xpath('//identity_cards/russian_passport[1]') }
    ...
    it { should have_tag('sex/code').with_value(true_sex_id) }
    it { should have_tag('sex/title').with_value(true_sex_title) }
    it { should have_tag('birth_place').with_value(true_birth_place) }
    it { should have_tag('birth_date').with_value(I18n.l(true_birth_date, :format => :xml)) }
  end
end
